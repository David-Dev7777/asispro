...
import { approveVacation, rejectVacation } from "@/lib/actions/admin/vacations"
...
function ReviewedBy({ request }: { request: VacationRequest }) {
...
export default function VacacionesClient({ initialRequests }: Props) {
  ...
  const handleReject = () => {
    ...
    setRejectError(null)
    startTransition(async () => {
      await rejectVacation(rejectId, rejectNote.trim())
      setRejectId(null)
      setRejectNote("")
      router.refresh()
    })
            ...
            {filtered.length === 0 ? (
              ...
              filtered.map(r => (
                <TableRow key={r.id}>
                  ...
                  <TableCell className="font-medium">{r.days_requested}d</TableCell>
